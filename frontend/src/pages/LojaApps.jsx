...
import { storeApps as initial } from "@/mock";
...
export default function LojaApps() {
  ...
  const install = (id) => {
    setApps((a) => a.map((x) => (x.id === id ? { ...x, installed: true } : x)));
            ...
            >
              <div className="flex items-start gap-4 mb-4">
                ...
                <div className="flex-1">
                  ...
                  <span className="inline-block mt-1 text-[10px] font-semibold tracking-wider uppercase bg-neutral-100 text-neutral-700 px-2 py-0.5 rounded">
                    {app.type}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-neutral-100">
                ...
                {app.installed ? (
                  <span className="inline-flex items-center gap-1.5 text-[13px] font-medium text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg">
                    <Check className="w-3.5 h-3.5" /> Instalado
                  </span>
                ) : (
                  <button
                    onClick={() => install(app.id)}
                    className="text-[13px] font-medium text-white bg-neutral-900 hover:bg-neutral-800 px-4 py-2 rounded-lg transition-colors"
                  >
                    Instalar
                  </button>
                )}
